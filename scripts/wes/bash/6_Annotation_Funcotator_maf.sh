...
#UCLA
#IN_DIR="/rds/projects/e/elhamsak-group5/main_project/processed_data/wes/variant_calling/ucla_processed_bed"
#OUT_DIR="/rds/projects/e/elhamsak-group5/main_project/processed_data/wes/annotation/ucla_processed_bed"

#Vanderbilt
IN_DIR="/rds/projects/e/elhamsak-group5/main_project/processed_data/wes/variant_calling/vanderbilt_processed_bed"
OUT_DIR="/rds/projects/e/elhamsak-group5/main_project/processed_data/wes/annotation/vanderbilt_processed_bed"
...
#Vanderbilt
TUMOUR_LIST=(
SRR4289728 SRR4289730 SRR4289732 SRR4289734 SRR4289736 SRR4289738 SRR4289740 SRR4289744
)

NORMAL_LIST=(
SRR4289727 SRR4289729 SRR4289731 SRR4289733 SRR4289735 SRR4289737 SRR4289739 SRR4289743
)
...
# --------------------
if [[ ! -f ${PREFIX_OUT}.annotated.maf ]]; then
    echo "Running Funcotator..."
    gatk Funcotator \
        -R "$REF_FASTA" \
        -V "$PASS_VCF" \
        --ref-version hg38 \
        --output "${PREFIX_OUT}.annotated.maf" \
        --data-sources-path "$FUNCOTATOR_DIR" \
        --output-file-format MAF \
        --java-options "-Xmx48G"
else
    echo "Annotated VCF already exists: ${PREFIX_OUT}.annotated.maf"
fi

# --------------------
# 2) Check Output Success
# --------------------
if [[ -f "${PREFIX_OUT}.annotated.maf" ]]; then
    echo "Annotation completed successfully: ${PREFIX_OUT}.annotated.maf"
else
    echo "Error: Annotated MAF was not created."
    exit 1
fi

# --------------------
# 3) MAF Header Validation
# --------------------
if grep -q "Hugo_Symbol" "${PREFIX_OUT}.annotated.maf"; then
    echo "MAF Validation: Found standard header."
else
    echo "MAF Validation: FAILED (Header missing)."
    exit 1
fi
